...
// ---------------------------------------------------------------------------
console.log('\n🧪 TESTE 6: PDF oficial — o portal ainda não oferece')
const pdfOficial = (await import('../api/nfse-download-pdf-oficial.js')).default
ok((await chamar(pdfOficial, { emission_id: 1 }, { auth: false })).code === 401, 'sem token → 401')
ok((await chamar(pdfOficial, {})).code === 400, 'sem emission_id → 400')
ok((await chamar(pdfOficial, { emission_id: 999999 })).code === 404, 'inexistente → 404')

if (!em?.chave_acesso) {
  console.log('  ⏭️  PULADO: nenhuma emissão com chave')
} else {
  const r = await chamar(pdfOficial, { emission_id: em.id })
  if (r.code === 200) {
    ok(r.buffer?.slice(0, 5).toString() === '%PDF-', 'o portal passou a oferecer o PDF — e é PDF de verdade')
  } else {
    // ⚠️ 503 com motivo, não 500 "falha ao baixar". A rota documentada
    // responde 501 (não implementada) e a /danfse responde 404: não é
    // instabilidade, não adianta tentar de novo, e o texto diz isso.
    ok(r.code === 503, `indisponível → 503 (não 500): ${r.body?.detalhe}`)
    ok(/não disponibiliza/.test(r.body?.error || ''), 'a mensagem explica que o portal não oferece')
    ok(Boolean(r.body?.alternativa), 'e aponta o DANFSE que geramos do XML autorizado')
    ok(Boolean(r.body?.rota_tentada), `diz qual rota tentou: ${r.body?.rota_tentada?.slice(0, 48)}…`)
  }
}
