...
export class NFSeADNClient {
  ...
  /**
   * Consulta a NFS-e pela chave de acesso e devolve o XML OFICIAL.
   *
   * ⚠️ Não existe `https://nfse.gov.br/api/v1/nfse/{chave}`: o host não
   * resolve, e `www.nfse.gov.br/api/v1/...` devolve a página 404 do portal.
   * A consulta é no SEFIN, no mesmo caminho da emissão, e exige o MESMO mTLS —
   * um GET sem certificado não passa do handshake.
   *
   * Verificado em produção: devolve
   * `{tipoAmbiente, versaoAplicativo, dataHoraProcessamento, chaveAcesso,
   *   nfseXmlGZipB64}`.
   */
  async consultarNFSe(chaveAcesso) {
    let resp
    try {
      resp = await this.client.get(`${ROTA_NFSE}/${encodeURIComponent(chaveAcesso)}`)
    } catch (err) {
      return { ok: false, status: null, erro: `Falha de comunicação com o SEFIN: ${err.message}`, resposta: null }
    }
    ...
    if (resp.status >= 300) {
      return {
        ok: false,
        status: resp.status,
        // 404 aqui é "ainda não disponível/inexistente", não erro nosso.
        erro: resp.status === 404
          ? 'NFS-e não encontrada no portal nacional'
          : NFSeADNClient.motivoDoErro(d, resp.status),
        resposta: d,
      }
    }
    ...
    if (d.nfseXmlGZipB64) {
      try {
        ...
        console.error('[sefin] não foi possível descompactar a NFS-e:', e.message)
      }
    }
    ...
    return {
      ok: true,
      status: resp.status,
      chaveAcesso: d.chaveAcesso ?? chaveAcesso,
      numeroNfse: nfseXml ? (nfseXml.match(/<nNFSe>([^<]+)<\/nNFSe>/) || [])[1] ?? null : null,
      nfseXml,
      resposta: { ...d, nfseXmlGZipB64: undefined },
    }
  }
