...
// ⚠️ O certificado da empresa pode ser REAL, enviado pela tela. Este teste
// NUNCA insere nem apaga um que já exista — o .pfx cifrado não fica guardado
// em outro lugar, e apagá-lo é irreversível.
//
// Esta guarda existe porque a falta dela já destruiu um certificado de verdade:
// o INSERT do fixture violava o UNIQUE(company_id), o teste morria, e o
// `finally` — que apagava sem condição — levava junto o certificado do Victor.
// A lição: bloco de limpeza só pode remover o que o próprio teste criou.
const certProprio = (await sql`SELECT id FROM nfse_certificates WHERE company_id = ${EMPRESA}`)[0] || null
const emitProprio = (await sql`SELECT id FROM nfse_emitter_settings WHERE company_id = ${EMPRESA}`)[0] || null
const jaTem = await sql`SELECT id FROM nfse_emissions WHERE invoice_id = ${INVOICE}`
if (jaTem.length || certProprio || emitProprio) {
  console.log(`  ⏭️  PULADO: empresa ${EMPRESA} já tem certificado/emitente/emissão — nada será tocado`)
} else {
  let em = null
  try {
    ;[em] = await sql`
    ...
    // Só o que ESTE teste criou. As duas guardas acima garantem que não havia
    // certificado nem emitente antes de ele rodar.
    ...
    if (!emitProprio) await sql`DELETE FROM nfse_emitter_settings WHERE company_id = ${EMPRESA}`
