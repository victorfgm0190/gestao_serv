...
// Contrato que não emite nota (hoje só a Minas). Mesmo predicado `temNf` de
// api/fiscal-obligations.js: só `false` exclui — coluna nula ou ausente é tributável,
// porque lançamento manual não tem fatura e não pode virar "sem NF" por omissão.
export const semNf = (rec) => rec?.require_nf === false

// Ordenação canônica do consumo. Idêntica ao preview do frontend (sortedPending em
// Financial.jsx) — as duas TÊM de andar juntas, senão a prévia mostra um consumo e a
// gravação faz outro.
//
// 1º  quem emite nota vem ANTES de quem não emite (decisão do Victor, 2026-08-12): a Minas
//     é recebida e paga à parte, então o saldo dela só é consumido quando não sobrou mais
//     nada — e nunca no meio dos outros clientes. ⚠️ Isto é ORDEM, não elegibilidade: o
//     payable dela continua elegível (consumir saldo é origem de caixa, não incidência de
//     tributo — ver CLAUDE.md), só foi para o fim da fila.
// 2º  competência ASC — mês mais antigo primeiro. Vale DENTRO de cada grupo: a Minas de
//     janeiro vem depois do Bokada de agosto, que é o que "processar depois" quer dizer.
// 3º  Pharmalog primeiro dentro do mês, restante por saldo desc.
//
// Nunca usa payment_month/year.
export function ordenar(records) {
  return [...records].sort((a, b) => {
    const na = semNf(a), nb = semNf(b)
...
export async function candidatosDisponiveis(sql, company_id, curKey) {
  const all = await sql`
    SELECT p.*, i.require_nf FROM payables_victor p
