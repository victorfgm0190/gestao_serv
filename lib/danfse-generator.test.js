...
ok(dps.prestador.razaoSocial === 'LUMEN DEV LTDA', 'razão social do prestador')
...
ok(dps.valores.servico === 340 && dps.valores.aliquotaIss === 2, 'valores lidos')
ok(dps.valores.iss === 6.8, `ISS derivado da alíquota: ${dps.valores.iss}`)
...
ok(contem('LUMEN DEV LTDA'), 'imprime a razão social do prestador')
...
if (jaTem.length) {
  ...
  try {
    ...
    const rx = await chamar(baixarXml, { emission_id: criada.id })
    ok(rx.code === 200, 'download XML → 200')
    ok(rx.headers['content-type']?.includes('application/xml'), 'Content-Type de XML')
    ok(/attachment; filename="NFSe_4242_nsu-teste\.xml"/.test(rx.headers['content-disposition']),
