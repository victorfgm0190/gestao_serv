...
const movimento = ({
  payable_id, client_id, month, year, source_type, destino, valor, notes, sem_debito = false,
}) => ({ payable_id, client_id, month, year, source_type, destino, valor: r2(valor), notes, sem_debito })
  ...
  return movimentos.map((m) => (m.sem_debito
    ...
    ? sql`
      INSERT INTO payment_sources
        (company_id, source_type, client_id, month, year,
         destination_category, amount, payment_id, notes, created_at, updated_at)
      VALUES (${company_id}, ${m.source_type}, ${m.client_id ?? null},
              ${m.month}, ${m.year}, ${m.destino}, ${m.valor}, NULL, ${m.notes ?? null},
              NOW(), NOW())`
    : sql`
    INSERT INTO payment_sources
      (company_id, source_type, client_id, month, year,
