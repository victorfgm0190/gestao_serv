...
class NFSeCertManager {
  ...
  async validateCertificate(sql, companyId, now = new Date()) {
    ...
    // A conta dos dias e os limiares moram em lib/nfse-cert-status.js — mesma
    // fonte que o GET da tela e o cron de alerta usam.
    const st = statusDoCertificado(
      cert.certificate_valid_until, now, cert.certificate_valid_from
    );
    ...
    const base = {
      ...
      daysRemaining: st.diasRestantes,
      status: st.status,
    };

    if (st.status === 'not_yet_valid') {
      return { ...base, valid: false, reason: 'Certificado ainda não é válido' };
    }
    if (st.status === 'expired') {
      return { ...base, valid: false, reason: 'Certificado expirado' };
    }
