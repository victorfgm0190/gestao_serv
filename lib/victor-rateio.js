...
const ORDEM_CATEGORIA = ['das', 'inss', 'honorarios', 'escritorio', 'pro_labore', 'lucros', 'demais']
...
export function planejarCategoria({ categoria, kind, valor, rateios, candidatos }) {
  const alocacoes = []
  ...
  const doKind = kind && KINDS_COM_RATEIO.has(kind)
    ? rateios.filter((r) => r.kind === kind).sort(ordemRateio)
    : []
  ...
  // ── PASSO 2: fallback ──────────────────────────────────────────────────────────────
  if (restante > FIM) {
    for (const rec of ordenarFallback(candidatos.filter((c) => c._saldo > FIM))) {
      if (restante <= FIM) break
...
export function planejar({ pagamentos, rateios, candidatos }) {
  ...
  for (const pg of ordenarCategorias(pagamentos)) {
    ...
    porCategoria.push({ categoria: pg.categoria, kind: pg.kind, valor: r2(pg.valor), ...r })
...
export function quitacoesPorObrigacao(porCategoria, obrigacoesPorKind, saldoDe) {
  ...
  for (const c of porCategoria) {
    ...
    if (!ob) continue
    const consumido = r2(c.alocacoes.reduce((s, a) => s + a.valor, 0))
