...
// Rateios das NOTAS informadas, sem passar pela competência.
//
// ⚠️ Existe porque a competência que a tela envia é a do FILTRO, e ela quase nunca é a da
// apuração: as NFs de janeiro são emitidas em fevereiro, então o rateio delas mora na
// competência 02. Com o filtro em janeiro, `buscarRateios(1, 2026)` volta vazio, o
// pagamento não acha fatia nenhuma, desce para o fallback e debita o SERVIÇO — o mesmo
// sintoma que a tradução de categoria já tinha produzido, por outro caminho.
//
// É a aplicação do princípio que abre este arquivo: A ÂNCORA É A NOTA, NÃO O MÊS. O card
// sabe exatamente quais notas está pagando (`invoice_ids`) e essa informação é mais
// confiável que qualquer mês derivado de um filtro de tela.
export async function buscarRateiosPorNotas(sql, company_id, invoiceIds) {
...
export function planejarCategoria({ categoria, kind, valor, rateios, candidatos, client_id = null, invoice_ids = null }) {
  ...
  // ── CASCATA DO IMPOSTO: rateio do kind → Lucro → Serviço ──────────────────────────
  //
  // Pagar "Honorários 150" consome PRIMEIRO a fatia que a apuração rateou para cada
  // cliente (Pharmalog 139,11 + Bokada 10,89 = os 150 exatos), e só o que passar disso
  // desce para o saldo do Victor.
  //
  // ⚠️ O consumo do rateio NÃO DEBITA O PAYABLE, e é isso que evita cobrar o imposto duas
  // vezes. Com a absorção ligada (Opção 2, 2026-08-15), o excedente do tributo JÁ foi
  // descontado de lucro/serviço pela redistribuição — o payable do Pharmalog já saiu de
  // 8.795,38 para 8.452,95. Debitá-lo de novo aqui tiraria o mesmo imposto do bolso dele
  // pela segunda vez. O que a linha do rateio registra é "esta fatia da guia foi paga", e
  // o saldo por cliente cai porque `victor-recorte` o deriva do `paid_amount` da obrigação.
  //
  // O EXCEDENTE sobre o rateio é outra coisa: dinheiro sem lastro fiscal (pagou-se mais do
  // que aquela competência devia). Esse desce para Lucro → Serviço debitando de verdade,
  // que é o passo 2 lá embaixo.
  if (quitaSemConsumir(categoria)) {
    const doKindSemDebito = kind && KINDS_COM_RATEIO.has(kind)
      ? rateios
    ...
    for (const rt of doKindSemDebito) {
      ...
      alocacoes.push({
        // `sem_debito` é o que separa esta linha das do passo 2: ela documenta de quem é a
        // fatia paga, sem tirar dinheiro do lançamento. agruparPorPayable() e o INSERT de
        // fiscal_allocations a ignoram — não há pagamento de payable a que se amarrar.
        tipo: 'rateio_fiscal',
        sem_debito: true,
        categoria,
        kind,
        ...
        payable_id: alvo?.id ?? null,
        invoice_id: rt.invoice_id,
        month: alvo?.month ?? null,
        year: alvo?.year ?? null,
        valor: cota,
        de_lucro: 0,
        de_servico: 0,
      })
      restante = r2(restante - cota)
    }

    // Tudo coberto pelo rateio: nada desce, nada é debitado.
    if (restante <= FIM) {
      return { alocacoes, restante: 0, rateios_sem_saldo, quitacao_direta: r2(valor), sem_consumo: true }
    }
    // Sobrou: o excedente segue para o passo 2 (Lucro → Serviço), debitando o payable.
    // `quitacao_direta` continua sendo o valor inteiro — a guia foi paga por completo,
    // independentemente de onde o dinheiro saiu.
  }

  // ── PASSO 1: rateios da categoria ──────────────────────────────────────────────────
  // Categoria de imposto já consumiu o rateio acima, SEM debitar — repetir aqui debitaria
  // o payable pela fatia que acabou de ser paga.
  const doKind = !quitaSemConsumir(categoria) && kind && KINDS_COM_RATEIO.has(kind)
    ? rateios
      .filter((r) => r.kind === kind)
      .filter((r) => alvoCliente == null || Number(r.client_id) === alvoCliente)
      .filter((r) => alvoNotas == null || alvoNotas.has(Number(r.invoice_id)))
  ...
  // Categoria de imposto quita a guia pelo que efetivamente saiu — rateio + o que o
  // fallback cobriu. Se nem o fallback deu conta, `restante` sobra e a guia é quitada só
  // pela parte paga: gravar o valor cheio marcaria como pago um dinheiro que não saiu.
  const extra = quitaSemConsumir(categoria)
    ? { quitacao_direta: r2(r2(valor) - r2(restante)), sem_consumo: true }
    : {}
  return { alocacoes, restante: r2(restante), rateios_sem_saldo, ...extra }
}
...
export function agruparPorPayable(alocacoes) {
  ...
  for (const a of alocacoes) {
    // Linha de rateio fiscal não debitou payable nenhum (ver planejarCategoria): gerar um
    // payable_payments para ela tiraria do Victor o imposto que a absorção já descontou.
