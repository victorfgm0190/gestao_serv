...
export class NFSeSigner {
   ...
   */
  assinarXML(xml, { elemento = 'infDPS', raiz = 'DPS' } = {}) {
    const { privateKeyPem, certificatePem } = extrairChaves(this.pfxBuffer, this.senha)

    const id = new RegExp(`<${elemento}\\s+Id="([^"]+)"`).exec(xml)?.[1]
    if (!id) throw new Error(`XML sem <${elemento} Id="...">: nada a referenciar na assinatura`)
