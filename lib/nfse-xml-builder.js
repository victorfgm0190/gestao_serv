...
export function montarIdDPS({ cLocEmi, cnpjEmitente, serie, nDPS }) {
  ...
  // ⚠️ 2 = CNPJ, 1 = CPF. Estava invertido, e o SEFIN recusava com E0004
  // ("identificador difere da concatenação dos campos"). Determinado por
  // tentativa contra a produção restrita: só a variante com `2` passa.
  const tpInsc = soDigitos(cnpjEmitente).length > 11 ? '2' : '1'
...
const OBRIGATORIOS = [
  ...
  // `inscricaoMunicipal` saiu dos obrigatórios: é opcional no Emissor Nacional.
 ...
 * o que preencher.
 */
// Comprimento mínimo de `xMotivo` (TSMotivo). Descoberto pelo schema: com
// "Correcao" (8 chars) o SEFIN recusa com "The actual length is less than the
// MinLength value".
...
export function camposFaltantes(dados) {
  ...
  if (dados?.substituicao) {
    const chave = soDigitos(dados.substituicao.chaveAcesso)
    if (chave.length !== 50) {
      faltando.push({
        campo: 'substituicao.chaveAcesso',
        rotulo: `Chave da nota a substituir deve ter 50 dígitos (tem ${chave.length})`,
      })
    }
    const motivo = String(dados.substituicao.motivo ?? '').trim()
    if (motivo.length < MOTIVO_MIN) {
      faltando.push({
        campo: 'substituicao.motivo',
        rotulo: `Motivo da substituição precisa de pelo menos ${MOTIVO_MIN} caracteres (tem ${motivo.length})`,
      })
    }
  }
...
export function montarDPS(dados) {
  ...
  // ⚠️ A DPS NÃO declara alíquota. O schema recusa `pAliq` dentro de
  // `tribMun` e lista o que aceita: cPaisResult, tpImunidade, exigSusp, BM,
  // tpRetISSQN. Faz sentido — quem apura o ISS é o município, a partir do
  // item da lista de serviços; o emitente informa a tributação, não a conta.
  // A alíquota cadastrada continua servindo para a previsão e para o DANFSE.
  //
  // tribISSQN 1 = operação tributável; tpRetISSQN 1 = não retido pelo tomador.
  const blocoIss =
    `<tribMun>${tag('tribISSQN', 1)}${tag('tpRetISSQN', valores.issRetido ? 2 : 1)}</tribMun>`
  ...
  return [
    ...
    // Substituição de NFS-e: a nova nota referencia a que ela substitui, e o
    // fisco faz o vínculo. Não é "cancelar e emitir outra" — isso perderia a
    // relação entre as duas.
    ...(dados.substituicao ? [
      '<subst>',
      tag('chSubstda', soDigitos(dados.substituicao.chaveAcesso)),
      tag('cMotivo', dados.substituicao.codigoMotivo || '01'),
      tag('xMotivo', dados.substituicao.motivo),
      '</subst>',
    ] : []),

    '<prest>',
    tag('CNPJ', soDigitos(emitente.cnpj)),
    // ⚠️ `tagOpcional`, não `tag`. Com a IM deixando de ser obrigatória, um
    // emitente sem ela produziria `<IM></IM>` — elemento vazio não casa com o
    // tipo do schema e a DPS é recusada. Sem IM, a tag simplesmente não sai.
    tagOpcional('IM', emitente.inscricaoMunicipal),
    // ⚠️ O bloco do prestador é MÍNIMO: só CNPJ e regime tributário. Tudo o
    // mais vem do cadastro nacional (CNC), e informar é rejeição:
    //   E0121 — nome/razão social do prestador não deve ser informado
    //   E0128 — endereço nacional do prestador não deve ser informado quando
    //           ele é o próprio emitente da DPS
    //   E0120 — IM não deve ser informada quando o município não tem
    //           informações complementares no CNC
    // Razão social e endereço continuam guardados em nfse_emitter_settings:
    // servem às telas e ao demonstrativo, não à DPS.
    tagOpcional('fone', soDigitos(emitente.telefone)),
    tagOpcional('email', emitente.email),
    '<regTrib>',
    tag('opSimpNac', emitente.optaSimples ?? 3),
    // ⚠️ E0166: optante do Simples (ME/EPP, opSimpNac 2 ou 3) é OBRIGADO a
    // declarar o regime de apuração. Fora do Simples o campo não se aplica.
    ...([2, 3].includes(Number(emitente.optaSimples ?? 3))
      ? [tag('regApTribSN', emitente.regimeApuracaoSN ?? 1)]
      : []),
    ...
    '<trib>',
    blocoIss,
    // ⚠️ E0712: para ME/EPP (optante do Simples) o `indTotTrib` NÃO pode ser
    // informado — o total de tributos da Lei da Transparência não se aplica.
    ...([2, 3].includes(Number(emitente.optaSimples ?? 3))
      ? [`<totTrib>${tag('pTotTribSN', decimal(valores.percentualTotalTributosSN ?? 0))}</totTrib>`]
