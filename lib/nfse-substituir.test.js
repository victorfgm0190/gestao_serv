...
const [fatura] = await sql`
  SELECT i.id FROM invoices i JOIN clients cl ON cl.id = COALESCE(i.invoice_client_id, i.client_id)
...
if (!emitente || !fatura) {
  ...
  try {
    ...
    // ⚠️ As operações saem ANTES das emissões e recortadas por INSTANTE:
    // nfse_operations.nfse_emission_id é ON DELETE SET NULL, então apagar a
    // emissão primeiro deixa a linha da trilha viva e sem vínculo. E o recorte
    // por company_id sozinho seria uma mina: a tabela guarda o histórico REAL
    // de envios ao SEFIN (é onde se investiga uma recusa), e um dia em que a
    // guarda de PULADO deixasse de valer, o teste apagaria essa trilha.
    await sql`DELETE FROM nfse_operations
               WHERE invoice_id = ${fatura.id} AND created_at >= ${inicio}`
