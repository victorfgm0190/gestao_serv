...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = await sql`SELECT p.*, c.name as client_name, r.amount as invoice_amount FROM payables_victor p LEFT JOIN clients c ON c.id = p.client_id LEFT JOIN receivables r ON r.invoice_id = p.invoice_id WHERE p.company_id = ${company_id} AND p.year = ${year} ORDER BY p.month DESC, p.created_at DESC`
