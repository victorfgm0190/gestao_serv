...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = await sql`SELECT p.*, c.name as client_name FROM payables_fabricio p LEFT JOIN clients c ON c.id = p.client_id WHERE p.company_id = ${company_id} AND p.year = ${year} ORDER BY p.month DESC, p.created_at DESC`
